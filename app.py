...
def download_log(name,ep_number):
    current_date = date.today()
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    with open("download_log.txt","a") as f:
        f.write("["+str(current_date) + ":" + current_time + " ] Downloaded " + name+": episode-"+ep_number+"\n")
    f.close()
        ...
        if((int(ep_num) >= 0) and (int(ep_num) <= last_ep)):
            ...
            if(x == "D" or x == "d"):
                ...
                if not os.path.exists(path):
                    os.makedirs(path)
                download_log(animes_to_watch,ep_num)
                download_episode(path,animes_to_watch,ep_num,last_ep)
            else:
                create_log(animes_to_watch,ep_num)
